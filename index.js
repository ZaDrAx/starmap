...
async function initialiserCartePublique() {
    ...
    img.onerror = function() {
        console.error("Image de fond introuvable :", mapImageUrl);
        calqueImageFond = L.imageOverlay(mapImageUrl, bounds).addTo(map);
        map.fitBounds(bounds);
        chargerCartePublique();
    };
...
function afficherSlide(index) {
    ...
    if(slides.length > 0) slides[currentSlide].classList.remove('active');
...
function construireCarrousel(photos, nomAstre) {
    ...
    photosActuelles.forEach((url, index) => {
        ...
        if (photosActuelles.length > 1) {
            ...
            dot.addEventListener('click', () => afficherSlide(index));
...
function ouvrirPanneau(donnees) {
    ...
    // Single KaTeX call, safely guarded
...
function appliquerFiltresLegende() {
    ...
    astresCharges.forEach(item => {
        ...
        if (typesCoches.includes(typeKey)) { if (!map.hasLayer(item.calque)) map.addLayer(item.calque); }
        else { if (map.hasLayer(item.calque)) map.removeLayer(item.calque); }
    });
}
...
async function chargerCartePublique() {
    try {
        ...
        querySnapshot.forEach((documentFirebase) => {
            ...
            let coords = [];
            try { coords = JSON.parse(astre.coordonnees); } catch(e) {}
            ...
            if (calque) {
                ...
                calque.on('tooltipopen', (e) => {
                    if (typeof renderMathInElement === 'function') {
                        renderMathInElement(e.tooltip._container, { delimiters: [{left:'$',right:'$',display:false}], throwOnError: false });
                    }
                });
                calque.on('click', (ev) => {
                    L.DomEvent.stopPropagation(ev);
                    ouvrirPanneau(astre);
                    const centre = astre.forme === 'polygone' ? calque.getBounds().getCenter() : L.latLng(coords[0], coords[1]);
        ...
        construireLegende();
    } catch (erreur) { console.error("Erreur chargement galaxies :", erreur); }
}