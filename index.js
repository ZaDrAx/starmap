...
const typesTraduction = {
    ...
    unknown: { fr: "Zone Spécifique / Contour", en: "Specific Area / Contour", es: "Área Específica / Contorno" }
};

document.getElementById('lang-selector').addEventListener('change', (e) => {
    currentLang = e.target.value;
    document.getElementById('message-accueil').innerText = lexique[currentLang].welcome; document.getElementById('lbl-legend-title').innerText = lexique[currentLang].legend;
    construireLegende();
...
function ouvrirPanneau(donnees) {
    ...
    const nomTraduit = donnees.nom[currentLang] || donnees.nom.fr; const descTraduite = donnees.description[currentLang] || donnees.description.fr;
    const cleType = donnees.typeAstre || "unknown"; const typeTraduit = typesTraduction[cleType] ? typesTraduction[cleType][currentLang] : typesTraduction.unknown[currentLang];

    document.getElementById('info-nom').innerText = nomTraduit; document.getElementById('info-type').innerText = typeTraduit; document.getElementById('info-description').innerText = descTraduite;
    ...
    const infoNom = document.getElementById('info-nom'); infoNom.setAttribute('tabindex', '-1'); infoNom.focus();
}
...
function appliquerFiltresLegende() {
    ...
    astresCharges.forEach(item => {
        ...
        const calqueDest = item.donnees.calqueAssigne || "principal";
        const calqueParentActif = (calqueDest === "principal" || (groupesFormesPublic[calqueDest] && map.hasLayer(groupesFormesPublic[calqueDest])));
        
        if (typesCoches.includes(typeKey) && calqueParentActif) {
            if (calqueDest === "principal" && !map.hasLayer(item.calque)) item.calque.addTo(map);
            else if (groupesFormesPublic[calqueDest] && !groupesFormesPublic[calqueDest].hasLayer(item.calque)) item.calque.addTo(groupesFormesPublic[calqueDest]);
        } else {
            if (map.hasLayer(item.calque)) map.removeLayer(item.calque);
            if (groupesFormesPublic[calqueDest] && groupesFormesPublic[calqueDest].hasLayer(item.calque)) groupesFormesPublic[calqueDest].removeLayer(item.calque);
        }
    });
}
